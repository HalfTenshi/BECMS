...
import { recomputeDenormForRelationField, recomputeDenormForTargetChange } from "../../services/denorm.service.js";

const router = express.Router();
router.use(auth, workspaceContext, authorize("UPDATE", "CONTENT_ENTRIES"));
...
router.post("/recompute/field", async (req, res) => {
  try {
    ...
    await recomputeDenormForRelationField({ workspaceId, relationFieldId, fromEntryIds });
    res.json({ ok: true });
  } catch (e) {
...
router.post("/recompute/target", async (req, res) => {
  try {
    ...
    await recomputeDenormForTargetChange({ workspaceId, targetEntryId });
    res.json({ ok: true });
  } catch (e) {
    res.status(400).json({ error: e.message });
  }
